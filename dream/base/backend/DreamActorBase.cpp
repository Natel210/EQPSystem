...
#define DEFINITION_DREAMACTORBASE
	...
	bool DreamActorBase::Initialize()
	{
		...
		reverseCheck |= !DreamObjectBase::Initialize();
	...
	bool DreamActorBase::Release()
	{
		...
		reverseCheck |= !DreamObjectBase::Release();
		if (!reverseCheck == true)
			Kill();
		return !reverseCheck;
	}

	std::shared_ptr<DreamObjectBase> DreamActorBase::Clone_ToObject() const
	{
	...
	void DreamActorBase::Progress_PrevUpdate()
	{
		...
		try
		{
			for (auto item : _componentList)
			{
				auto temp = std::dynamic_pointer_cast<ILogic>(item.second);
				if (temp == nullptr)
					continue;
				temp->PrevUpdate();
			}
